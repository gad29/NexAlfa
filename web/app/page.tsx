...
export default function NexAlfa() {
  ...
  useEffect(() => {
    ...
    const GATEWAY = process.env.NEXT_PUBLIC_GATEWAY_URL || 'http://localhost:18789';
...
function SkillsView() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_GATEWAY_URL || "http://localhost:18789"}/api/skills`)
...
function SettingsView({ status }: { status: AgentStatus | null }) {
  ...
  const [toast, setToast] = useState("");
  const API = process.env.NEXT_PUBLIC_GATEWAY_URL || "http://localhost:18789";
